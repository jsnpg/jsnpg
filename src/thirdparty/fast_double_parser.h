...
//
// Apache LICENSE
// https://github.com/lemire/fast_double_parser/blob/master/LICENSE
//
// BOOST LICENSE 
// https://github.com/lemire/fast_double_parser/blob/master/LICENSE.BST
//
...
static really_inline uint64_t Emulate64x64to128(uint64_t *r_hi, const uint64_t x, const uint64_t y) {
...
static really_inline struct value128 full_multiplication(uint64_t value1, uint64_t value2) {
  struct value128 answer;
...
static really_inline double compute_float_64(int64_t power, uint64_t i, bool negative,
                                      bool *success) {
  ...
  // 1.
  struct value128 product = full_multiplication(i, factor_mantissa);
...
static char *parse_float_strtod(const char *ptr, double *outDouble) {
  ...
  char *endptr;
  char *curr_locale = setlocale(LC_NUMERIC, NULL);
